...
export function applyEffectToEnemies(applyFlag, shapeMode, maxTargets, effectType, effectDuration, damageMin, damageMax, centerPos, directionVec, width, height) { // al
    ...
    for (let enemyIdx = 0; enemyIdx < EnemyState.enemyCount; enemyIdx++) {
        ...
        if (0 > EnemyState.enemyHealthArray[enemyIdx]) // dead
            continue;

        let jpos = EnemyState.enemyJointPosArray[enemyIdx][EnemyState.enemyTargetJointIdx];
        let hbw = enemyHitboxHalfWidthByBehavior[EnemyState.enemyUpdateFuncIdxArray[enemyIdx]] * enemyCatalog[EnemyState.enemyTypeArray[enemyIdx]][EnemyProps.DrawScale];
        let hbh = enemyHitboxHalfHeightByBehavior[EnemyState.enemyUpdateFuncIdxArray[enemyIdx]] * enemyCatalog[EnemyState.enemyTypeArray[enemyIdx]][EnemyProps.DrawScale];
        if (EnemyState.enemyUpdateFuncIdxArray[enemyIdx] == BehaviorTypes.TreeLeft || 
            ...
            hbh = 3 * EnemyState.enemyPoseTrailWriteIdxArray[enemyIdx] + 5 * enemyCatalog[EnemyState.enemyTypeArray[enemyIdx]][EnemyProps.DrawScale];
        
        if (jpos.x - hbw > drx || jpos.x + hbw < tlx || jpos.y - hbh > dry || jpos.y + hbh < tly) // outside of hitbox
            continue;
        ...
        if (0 == shapeMode) {
            ...
            for (i = 0; i <= scanLength; i++) {
                let tileIdx = getStageTileAt(scanPos.x, scanPos.y);
                if (0 <= tileIdx && 29 >= tileIdx) break;
                if (jpos.x - hbw < scanPos.x && jpos.x + hbw > scanPos.x && jpos.y - hbh < scanPos.y && jpos.y + hbh > scanPos.y) {
                    i = scanLength + 2;
                    break;
                }
                scanPos.add(scanDir);
        ...
        if (0 == applyFlag) {
            ...
            if (4 == effectType) {
                ...
                spawnPopup(EnemyState.enemyJointPosArray[enemyIdx][EnemyState.enemyTargetJointIdx].x, EnemyState.enemyJointPosArray[enemyIdx][EnemyState.enemyTargetJointIdx].y - hbh, 0 > scanDir.x ? -1 : 1, dmg, 60, 12632256);
...
export function enemySlimeBehavior(enemyIdx) {
    ...
    if (0 == EnemyState.enemyPoseTrailWriteIdxArray[enemyIdx]) {
        ...
        // for (let i = 0; 1 > i; i++) 
        EnemyState.enemyPrevJointPosArray[enemyIdx][0].set(EnemyState.enemyJointPosArray[enemyIdx][0]);
        ...
        let d = enemySpriteAnchorYBySpriteIndex[enemyCatalog[EnemyState.enemyTypeArray[enemyIdx]][EnemyProps.SpriteIndex]];
        spawnEnemyLoot(enemyIdx, 0, EnemyState.enemyJointPosArray[enemyIdx][0].x, EnemyState.enemyJointPosArray[enemyIdx][0].y - d * drawScale + 1);
        EnemyState.enemyTileContactFlagsArray[enemyIdx] = 0;
        if (0 >= EnemyState.enemyHealthArray[enemyIdx])
            for (let i = 0; 1 > i; i++) {
                EnemyState.enemyJointPosArray[enemyIdx][i].x += RMath.randFloatRange(-.3, .3);
                EnemyState.enemyJointPosArray[enemyIdx][i].y -= RMath.randFloatRange(1, 2);
            }
        for (let i = 0; 1 > i; i++) moveEnemyJointWithTileCollision(enemyIdx, i, .5);
        EnemyState.enemyJointPosArray[enemyIdx][EnemyState.enemyTargetJointIdx].x = EnemyState.enemyJointPosArray[enemyIdx][0].x;
        EnemyState.enemyJointPosArray[enemyIdx][EnemyState.enemyTargetJointIdx].y = EnemyState.enemyJointPosArray[enemyIdx][0].y - d * drawScale + 1;
        if (0 >= EnemyState.enemyHealthArray[enemyIdx]) {
            EnemyState.enemyPoseTrailWriteIdxArray[enemyIdx] = 3;
            onEnemyDeath(enemyIdx);
        }
    } else {
        for (let i = 0; 1 > i; i++) RMath.stepWithVerticalBias(EnemyState.enemyJointPosArray[enemyIdx][i], EnemyState.enemyPrevJointPosArray[enemyIdx][i], .05, .99);
        for (let i = EnemyState.enemyTileContactFlagsArray[enemyIdx] = 0; 1 > i; i++) moveEnemyJointWithTileCollision(enemyIdx, i, .5);
